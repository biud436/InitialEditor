...
export function ContentView({
  ...
  return (
    <Box
      className={classNames('tilesetWindow-tile', 'tilesetWindow__tab-border')}
      tab-name="타일"
    >
      <ContentViewContainer>
        <ListItem key="tileset-window-tile-width">
          <Label htmlFor="tile-width">가로 크기: </Label>
          <Input type="number" id="tile-width" value="32" name="tileWidth" />
          px
        </ListItem>
        <ListItem key="tileset-window-tile-height">
          <Label htmlFor="tile-height">세로 크기: </Label>
          <Input type="number" id="tile-height" value="32" name="tileHeight" />
          px
        </ListItem>
        <ListItem key="tileset-window-theme-setting">
          ...
          >
            <OptionItem value="dark" selected={theme.theme === 'dark'}>
              다크 테마
            </OptionItem>
            <OptionItem value="light" selected={theme.theme === 'light'}>
              라이트 테마
            </OptionItem>
          </Select>
        </ListItem>
